...
RawImage* LoadJpegFromPath(const char* filename)
{
...
#if DEBUG
		fprintf(stderr,
				"[ERROR] <%s:%u> Failed to read image from file %s\n",
				__FILE__,
				__LINE__,
				filename);
#endif
...
#if DEBUG
	fprintf(stderr,
			"[INFO] <%s> Reading JPEG with size (%lu x %lu) and %u color channels\n",
			filename,
			imgWidth,
			imgHeight,
			imgNumColorChannels);
	...
	while(info.output_scanline < info.output_height)
	{
		...
		imgRowBuffer[0] = (unsigned char*)(&newImage->data[info.num_components * info.output_width *
...
RawImage* LoadPngFromPath(const char* filename)
{
...
#if DEBUG
		fprintf(stderr,
				"[ERROR] <%s:%u> File %s could not be opened for reading.\n",
				__FILE__,
				__LINE__,
				filename);
#endif
...
#if DEBUG
		fprintf(stderr,
				"[ERROR] <%s:%u> File %s is not recognized as a PNG file.\n",
				__FILE__,
				__LINE__,
				filename);
#endif
	...
	RawImage* outputImage = LoadRawImage(png_get_image_width(pngptr, infoptr),
...
#if DEBUG
	fprintf(stderr,
			"[INFO] <%s> Reading PNG with size (%lu x %lu) and %u color channels\n",
			filename,
			outputImage->width,
			outputImage->height,
			outputImage->numColorChannels);
#endif
...
RawImage* LoadRawImage(unsigned long width, unsigned long height, unsigned int numColorChannels)
{
	...
	outImage->data =
		(unsigned char*)malloc(sizeof(unsigned char) * width * height * numColorChannels);
	if(!outImage->data)
	{
#if DEBUG
		fprintf(stderr,
				"[ERROR] <%s:%u> Could not allocate memory for RawImage databuffer!\n",
				__FILE__,
				__LINE__);
#endif
		free(outImage);
...
#if DEBUG
		fprintf(
			stderr, "[ERROR] <%s:%u> Malloc failed for NormImage object!\n", __FILE__, __LINE__);
#endif
		return NULL;
	}
	outImage->width = width;
...
#if DEBUG
		fprintf(stderr,
				"[ERROR] <%s:%u> Could not allocate memory for NormImage databuffer!\n",
				__FILE__,
				__LINE__);
#endif
		free(outImage);
