...
NormImage* NormalizeRawImage(RawImage* inputImage)
{
	...
	outputImage =
		LoadNormImage(inputImage->width, inputImage->height, inputImage->numColorChannels);
	if(!outputImage)
	{
#if DEBUG
	...
	NormImage* inputImage, double angles[], int nangles, int nbins, int color, int resolution)
{
	...
	// hyp);
	double separation =
		sqrt(inputImage->width * inputImage->width + inputImage->height * inputImage->height) /
		nbins;
	...
	for(int anglei = 0; anglei < nangles; ++anglei)
	{
		...
		for(int pixi = 0; pixi < inputImage->width * inputImage->height; ++pixi)
		{
			...
			for(int subpixi = 0; subpixi < resolution * resolution; ++subpixi)
			{
				...
				// if (pixi == 204906) fprintf(stderr, "[%lf %lf] ", projVal, bini);
				line_len = lenImage->data[anglei * inputImage->height * inputImage->width *
