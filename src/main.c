...
int main()
{
	...
	RawImage* imageData =
		LoadJpegFromPath(imageFile); /* Path is relative to directory program runs from */
	if(!imageData)
	{
#if DEBUG
	...
	double newRatio;
	if(fabs((2. * n) / (totalNails - 2. * n) - origRatio) <
	   fabs((2. * n + 2.) / (totalNails - 2. * n - 2.) - origRatio))
	{
...
#if DEBUG
	fprintf(stderr,
			"[INFO] OrigRatio=%lf, NewRatio=%lf, NewDims=(%lu x %lu), baseNails=%u heightNails=%u, "
			"seperation=%lf\n",
			origRatio,
			newRatio,
			newWidth,
	...
	for(int rowi = rowStart; rowi < rowStart + newHeight; ++rowi)
	{
		for(int coli = colStart; coli < colStart + newWidth; ++coli)
		{
			...
			{
				image->data[((rowi - rowStart) * newWidth + coli - colStart) *
								imageData->numColorChannels +
							color] =
					imageData
						->data[((rowi * imageData->width) + coli) * imageData->numColorChannels +
							   color];
			}
		}
	}
	...
	{
	case 1:
		invImage = NormalizeRawImage(image);
		break;
	case 3:
		invImage = RgbToPrintable(image);
		break;
	default:
		invImage = NULL;
		break;
...
#if DEBUG
		fprintf(stderr,
				"[ERROR] <%s:%u> Failed to Normalize image! Try checking the number of color "
